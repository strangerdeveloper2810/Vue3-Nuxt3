  ...
  >
    ...
    <div class="relative overflow-hidden">
      ...
      <div class="absolute top-2 left-2 flex flex-col gap-1">
        ...
        <span
          v-if="movie.hot"
          class="bg-orange-500 text-white text-xs px-2 py-1 rounded-full font-medium"
        >
          🔥 Hot
        </span>
      </div>
      ...
      <!-- Rating -->
      <div
        class="absolute top-2 right-2 bg-black bg-opacity-70 rounded-full px-2 py-1"
      >
    ...
    <div class="p-4">
      <!-- Title -->
      <NuxtLink :to="`/${movie.maPhim}`" class="block cursor-pointer">
        <h3
          class="text-white font-semibold text-lg mb-2 line-clamp-2 group-hover:text-red-400 hover:text-red-400 transition-colors duration-200"
        >
          {{ movie.tenPhim }}
        </h3>
      </NuxtLink>
      ...
      <div class="flex gap-2">
        ...
        >
          <UIcon
            :name="
              isInWishlist ? 'i-heroicons-heart-solid' : 'i-heroicons-heart'
            "
            class="w-4 h-4"
          />
        </UButton>
    ...
    <div
      v-if="loading"
      class="absolute inset-0 bg-gray-800 bg-opacity-90 flex items-center justify-center"
    >
      <UIcon
        name="i-heroicons-arrow-path"
        class="w-6 h-6 animate-spin text-red-500"
      />
    </div>
  </div>
</template>
